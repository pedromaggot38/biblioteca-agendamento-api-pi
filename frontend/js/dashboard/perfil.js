document.addEventListener('DOMContentLoaded', () => {
  ...
  btnGerar?.addEventListener('click', async () => {
    ...
    const estaEmModoTroca = containerNovoEmail.style.display === 'block';
    ...
    if (estaEmModoTroca) {
      ...
      if (novoEmailDigitado === emailAtual) {
        return mostrarNotificacao(
          'Este já é o seu e-mail atual. Para verificar, apenas envie sem abrir a edição.',
          'aviso',
        );
      }
      emailParaEnviar = novoEmailDigitado;
    }
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/users/me/request-verification`, {
        ...
        body: JSON.stringify({ novoEmail: emailParaEnviar }),
      });
      ...
      if (btnHabilitarTroca) btnHabilitarTroca.style.display = 'none';
